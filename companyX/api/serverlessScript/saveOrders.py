...
def query_database(body):
	...
	try:
		with connection.cursor() as cursor:
			...
			sql = "SELECT * FROM partOrdersX WHERE jobName = %s AND partId = %s AND userId = %s"
			cursor.execute(sql, (jobName, int(partId), int(userId)) )
			result = cursor.fetchone()
			
			if(result is None):
				with connection.cursor() as cursor:
					sql = "INSERT INTO partOrdersX VALUES (%s, %s, %s, %s)"
					cursor.execute(sql, (jobName, int(partId), int(userId), int(qty)) )
					connection.commit()
